...
# Make all submodules available at the package level
...
__all__ = [
    'convert', 'merge', 'edit_pdf', 'compress', 'drag_drop',
    'magick', 'split', 'developer', 'check_dependencies',
    'image_tool', 'style', 'pdf_viewer'
]

# Try to ensure all modules are importable
def ensure_modules_importable():
    """Ensure all utils modules can be imported regardless of packaging method."""
    if getattr(sys, 'frozen', False):
        # We're in a frozen/PyInstaller environment
        base_dir = getattr(sys, '_MEIPASS', os.path.dirname(sys.executable))
        package_dir = os.path.join(base_dir, 'utils')
        
        # Add the utils directory to sys.path if it exists
        ...
        # Check for the tabs directory too
        tabs_dir = os.path.join(base_dir, 'tabs')
        if os.path.isdir(tabs_dir) and tabs_dir not in sys.path:
